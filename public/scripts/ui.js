...
Events.on('display-name', e => {
	...
	const $cRID = $('cRID');
	$cRID.addEventListener('click', e => {
		if ($cRID.getAttribute('active') === 'true') {
			$cRID.setAttribute('active', 'false');
			$cRID.textContent = '自定义房间号';
			$roomID.contentEditable = false;
			snapdrop.bug.send({ type: 'updateRID', rid: $roomID.textContent });
		} else {
			$cRID.setAttribute('active', 'true');
			$cRID.textContent = '在上方输入新的房间号后点击我确认';
